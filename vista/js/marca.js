...
function limpiar() {
    ...
    $("br").css("display", "none");
    $("#NmMarca").css("background-color", "rgb(255, 255, 255)");
...
function listar() {
    ...
    tabla = $('#tbllistado').dataTable({
        ...
        "ajax": {
            url: '../controlador/marca.php?op=listar',
            type: "get",
            dataType: "json",
            error: function(e) {}
        },
...
$(document).ready(function() {
    ...
    $("#btnGuardar").click(function(e) { //Valida que ningun campo esté vacio antes de guardar
        ...
        if ($.trim($("#NmMarca").val()).length == 0) {
            console.log('campos vacíos');
            $("#NmMarca").css("background-color", "rgba(179, 27, 27, 0.096)");
            $(".infoC").css("display", "inline"); //.fadeOut(1000); .hiden(2000);
        } else {
            $("#NmMarca").css("background-color", "rgba(255, 255, 255, 0.911)");
            $(".infoC").css("display", "none");
            guardar(e);
        }
    });
...
//     if ($.trim($("#NmMarca").val()).length == 0) {
//         console.log('campo NmMarca vacío');
...
function guardar(e) {
    ...
    $.ajax({
        url: "../controlador/marca.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function editar(e) {
    ...
    //console.log(formData);
    $.ajax({
        url: "../controlador/marca.php?op=editar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(IdMarca) {
    $.post("../controlador/marca.php?op=mostrar", { IdMarca: IdMarca }, function(data, _status) {
        data = JSON.parse(data);
        mostrarEditar(true);
        $('#IdMarca').val(IdMarca);
        $('#NmMarca').val(data.NmMarca);
    })
}
...
function eliminar(IdMarca) {
    swal({
        ...
        if (isConfirm) {
            $.post("../controlador/marca.php?op=eliminar", { IdMarca: IdMarca }, function(e) {
