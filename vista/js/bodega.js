...
function limpiar() {
    ...
    $("br").css("display", "none");
    $("#NmBodega").css("background-color", "rgb(255, 255, 255)");
    $(".infoC").css("display", "none");
    $("#FlActiva").css("background-color", "rgb(255, 255, 255)");
...
function listar() {
    ...
    tabla = $('#tbllistado').dataTable({
        ...
        "ajax": {
            url: '../controlador/bodega.php?op=listar',
            type: "get",
            dataType: "json",
            error: function(e) {}
        },
...
$(document).ready(function() {
    ...
    $('#IdBodega').keyup(function() {
        ...
        var dataString = 'IdBodega=' + IdBodega;

        $.ajax({
            type: "POST",
            url: "../controlador/bodega.php?op=datosid",
            data: dataString,
            success: function(data) {
                ...
                if (data.trim() !== "\r\nnull" || data.trim() !== undefined || data.trim() !== null || data.trim() !== "null") {
                    ...
                    $('#NmBodega').val(data.NmBodega);
                    $('#FlActiva').val(data.FlActiva);
        ...
        $.ajax({
            type: "POST",
            url: "../controlador/bodega.php?op=validarid",
            data: dataString,
            success: function(data) {
                console.log(data);
                if (data.trim() === "\r\nnull" || data.trim() == undefined || data.trim() == null || data.trim() == "null") {
    ...
    $("#btnGuardar").click(function(e) { //Valida que ningun campo esté vacio antes de guardar
        ...
        if ($.trim($("#NmBodega").val()).length == 0 || $.trim($("#FlActiva").val()).length == 0) {
...
function campos() {
    ...
    if ($.trim($("#NmBodega").val()).length == 0) {
        console.log('campo NmBodega vacío');
        $("#NmBodega").css("background-color", "rgba(179, 27, 27, 0.096)");
        $(".infoC").css("display", "inline"); //.fadeOut(1000); .hiden(2000);
    } else {
        $("#NmBodega").css("background-color", "rgba(255, 255, 255, 0.911)");
        $(".infoC").css("display", "none");
    }

    if ($.trim($("#FlActiva").val()).length == 0) {
        console.log('campo FlActiva vacío');
        $("#FlActiva").css("background-color", "rgba(179, 27, 27, 0.096)");
        $(".infoA").css("display", "inline");
    } else {
        $("#FlActiva").css("background-color", "rgba(255, 255, 255, 0.911)");
        $(".infoA").css("display", "none");
    }
...
function guardar(e) {
    ...
    $.ajax({
        url: "../controlador/bodega.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function editar(e) {
    ...
    //console.log(formData);
    $.ajax({
        url: "../controlador/bodega.php?op=editar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(IdBodega) {
    $.post("../controlador/bodega.php?op=mostrar", { IdBodega: IdBodega }, function(data, _status) {
        data = JSON.parse(data);
        mostrarEditar(true);
        $('#IdBodega').val(IdBodega);
        $('#NmBodega').val(data.NmBodega);
...
function eliminar(IdBodega) {
    swal({
        ...
        if (isConfirm) {
            $.post("../controlador/bodega.php?op=eliminar", { IdBodega: IdBodega }, function(e) {
