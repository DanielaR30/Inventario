...
function limpiar() {
    ...
    $("br").css("display", "none");
    $("#NmLinea").css("background-color", "rgb(255, 255, 255)");
...
function listar() {
    ...
    tabla = $('#tbllistado').dataTable({
        ...
        "ajax": {
            url: '../controlador/linea.php?op=listar',
            type: "get",
            dataType: "json",
            error: function(e) {}
        },
...
$(document).ready(function() {
    ...
    $("#btnGuardar").click(function(e) { //Valida que ningun campo esté vacio antes de guardar
        ...
        if ($.trim($("#NmLinea").val()).length == 0) {
            console.log('campos vacíos');
            $("#NmLinea").css("background-color", "rgba(179, 27, 27, 0.096)");
            $(".infoC").css("display", "inline"); //.fadeOut(1000); .hiden(2000);
        } else {
            $("#NmLinea").css("background-color", "rgba(255, 255, 255, 0.911)");
            $(".infoC").css("display", "none");
            guardar(e);
        }
    });
...
//     if ($.trim($("#NmLinea").val()).length == 0) {
//         console.log('campo NmLinea vacío');
...
function guardar(e) {
    ...
    $.ajax({
        url: "../controlador/linea.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function editar(e) {
    ...
    //console.log(formData);
    $.ajax({
        url: "../controlador/linea.php?op=editar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(IdLinea) {
    $.post("../controlador/linea.php?op=mostrar", { IdLinea: IdLinea }, function(data, _status) {
        data = JSON.parse(data);
        mostrarEditar(true);
        $('#IdLinea').val(IdLinea);
        $('#NmLinea').val(data.NmLinea);
    })
}
...
function eliminar(IdLinea) {
    swal({
        ...
        if (isConfirm) {
            $.post("../controlador/linea.php?op=eliminar", { IdLinea: IdLinea }, function(e) {
