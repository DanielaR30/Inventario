...
function limpiar() {
    ...
    $("#NmEstadoProducto").val("");
    $(".infoguar").css("display", "none");
    $("br").css("display", "none");
    $("#NmEstadoProducto").css("background-color", "rgb(255, 255, 255)");
...
function listar() {
    ...
    tabla = $('#tbllistado').dataTable({
        ...
        "ajax": {
            url: '../controlador/estadoproducto.php?op=listar',
            type: "get",
            dataType: "json",
            error: function(e) {}
        },
...
$(document).ready(function() {
    ...
    $("#btnGuardar").click(function(e) { //Valida que ningun campo esté vacio antes de guardar
        ...
        if ($.trim($("#NmEstadoProducto").val()).length == 0) {
...
function campos() {
    ...
    if ($.trim($("#NmEstadoProducto").val()).length == 0) {
        console.log('campo NmEstadoProducto vacío');
        $("#NmEstadoProducto").css("background-color", "rgba(179, 27, 27, 0.096)");
        $(".infoA").css("display", "inline"); //.fadeOut(1000); .hiden(2000);
    } else {
        // $("#IdUsuario").focus(function(){
        $("#NmEstadoProducto").css("background-color", "rgba(255, 255, 255, 0.911)");
        $(".infoA").css("display", "none");
        // });
    }
}
...
function guardar(e) {
    ...
    $.ajax({
        url: "../controlador/estadoproducto.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function editar(e) {
    ...
    //console.log(formData);
    $.ajax({
        url: "../controlador/estadoproducto.php?op=editar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(IdEstadoProducto) {
    $.post("../controlador/estadoproducto.php?op=mostrar", { IdEstadoProducto: IdEstadoProducto }, function(data, _status) {
        data = JSON.parse(data);
        mostrarEditar(true);
        $('#IdEstadoProducto').val(IdEstadoProducto);
        $('#NmEstadoProducto').val(data.NmEstadoProducto);
    })
}

function eliminar(IdEstadoProducto) {
    swal({
        title: "¿Eliminar?",
        ...
        if (isConfirm) {
            $.post("../controlador/estadoproducto.php?op=eliminar", { IdEstadoProducto: IdEstadoProducto }, function(e) {
